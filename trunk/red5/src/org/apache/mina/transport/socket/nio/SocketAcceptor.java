...
public class SocketAcceptor extends BaseIoAcceptor {
   ...
   * connections with the specified <code>handler</code>. Backlog value is
   * configured to the value of <code>backlog</code> property.
   * 
   * @throws IOException
   *           if failed to bind
   */
  ...
  private class Worker implements Runnable {
    public void run() {
      ...
      for (;;) {
        ThreadMonitorServices.start("SocketAcceptor.Worker.run())");
        ThreadMonitorServices.setJobType("thread");
        try {
          ThreadMonitorServices.startJobSubDetails();
          ThreadMonitorServices.setJobDetails("selector.select()");
          ...
          ThreadMonitorServices.startJobSubDetails();
          ThreadMonitorServices.setJobDetails("exception caught {0}", e);
          ExceptionMonitor.getInstance().exceptionCaught(e);
    ...
    private void processSessions(Set<SelectionKey> keys) throws IOException {
      ThreadMonitorServices.startJobSubDetails();
      ThreadMonitorServices.setJobDetails("processSessions(Set<SelectionKey> keys {0})", keys);
      ...
      while (it.hasNext()) {
        ...
        try {
          ...
          SocketSessionImpl session = new SocketSessionImpl(SocketAcceptor.this, nextProcessor(), getListeners(),
  ...
  private void registerNew() {
    ThreadMonitorServices.startJobSubDetails();
    ThreadMonitorServices.setJobDetails("registerNew()");
  ...
  private void cancelKeys() {
    ThreadMonitorServices.setJobDetails("cancelKeys()");
    ...
    for (;;) {
      ...
      try {
        ...
        if (request.exception == null) {
          getListeners().fireServiceDeactivated(this, request.address, request.registrationRequest.handler,

...
public class SharedObjectScope extends BasicScope implements ISharedObject, StatusCodes {
  ...
  /**
   * Creates shared object with given parent scope, name, persistence flag state
   * and store object
   * 
   * @param parent
   *          Parent scope
  ...
  public void beginUpdate(IEventListener listener) {
    ThreadMonitorServices.startJobSubDetails();
    ThreadMonitorServices.setJobDetails("SharedObjectScope.beginUpdate(IEventListener listener) {0}", listener);
    // Make sure only one thread can update the SO
    lock.lock();
  ...
  public void sendMessage(String handler, List arguments) {
    ThreadMonitorServices.startJobSubDetails();
    ThreadMonitorServices.setJobDetails("SharedObjectScope.sendMessage(String handler {0}, List arguments {1})",
        handler, arguments);
    // counting chat time
    ...
    if (soHandler == null && hasParent()) {
      ...
      try {
        ...
        if (!so.getName().startsWith("loc") && !so.getName().startsWith("farm")
            && !so.getName().startsWith("admin_shared") && !so.getName().startsWith("|game")
            && !so.getName().startsWith("game") && !so.getName().startsWith("home")
    ...
    ThreadMonitorServices.stopJobSubDetails("SharedObjectScope.sendMessage(String handler {0}, List arguments {1})",
        handler, arguments);
  }

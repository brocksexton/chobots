package com.kavalok.questSummer
{
	...
	public class SantaCommand extends NPCCommand
	{
		...
		override protected function onNpcActivate():void
		{
			...
			else if (state == QuestStates.QUEST_TELEPORT)
			{
				dialog.onClose = loadChlos;
				location.sendUserTool(null);
			}
		...
		private function giveItem():void
		{
			...
			Global.moduleManager.loadModule(Modules.CHLOS);
		} else {
			Dialogs.showOkDialog("Sorry, this feature is available only for citizens.", true);
		}
		}
		
		private function loadChlos():void
		{
			if (Global.charManager.isCitizen){
			Global.moduleManager.loadModule(Modules.CHLOS);
		} else {

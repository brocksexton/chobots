package com.kavalok.admin.infoPanel
{
	...
	public class InfoItemBase extends Box
	{
		...
		protected function onSaveClick():void
		{
			...
			item.data = toXML();
			if(item.enabled)
			new LogService().adminLog("Enabled info panel " + _infoItem.id + " with caption: " + captionInput.text, 1, "info panel");
			else
			new LogService().adminLog("Modified info panel " + _infoItem.id, 1, "info panel");

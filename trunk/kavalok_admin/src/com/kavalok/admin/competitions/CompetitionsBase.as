package com.kavalok.admin.competitions
{
	...
	public class CompetitionsBase extends HBox
	{
		...
		internal function onStartedChange(event : Event) : void
		{
			...
			new LogService().adminLog("Started competition " + competition.name + " until " + finish, 1, "competition");
			else
			new LogService().adminLog("Ended competition " + competition.name, 1, "competition");
		}
		...
		internal function onCompetitionClear(event : MouseEvent) : void
		{
			...
			new LogService().adminLog("Cleared competition " + data.name, 1, "competition");
		}

...
public class ExecutorFilter extends IoFilterAdapter {
  ...
  protected void processEvent(NextFilter nextFilter, IoSession session, EventType type, Object data) {
    ...
    ThreadMonitorServices
        .stopJobSubDetails(
            "ExecutorFilter.processEvent(NextFilter nextFilter {0}, IoSession session {1}, EventType type {2}, Object data {3})",
            nextFilter, session, type, data);
  }
  ...
  private class ProcessEventsRunnable implements Runnable {
    ...
    public void run() {
      while (true) {
        ...
        ThreadMonitorServices
            .start("org.apache.mina.filter.executor.ExecutorFilter.ProcessEventsRunnable.run() events waiting: "
                + buffer.eventQueue.size());
        ThreadMonitorServices.setJobType("executor-filter");

...
class SocketIoProcessor {
   ...
   * {@link #write(IoSession, IoBuffer)} returns non-zero value. It is similar
   * to what a spin lock is for in concurrency programming. It improves memory
   * utilization and write throughput significantly.
   */
  ...
  private void doAddNew() {
    ThreadMonitorServices.setJobDetails("doAddNew()");
    Selector selector = this.selector;
    for (;;) {
  ...
  private void doRemove() {
    ThreadMonitorServices.setJobDetails("doRemove()");
    for (;;) {
  ...
  private void process(Set<SelectionKey> selectedKeys) {
    ThreadMonitorServices.setJobDetails("process((Set<SelectionKey> selectedKeys {0}))", selectedKeys);
  ...
  private void notifyIdleness() {
    ThreadMonitorServices.setJobDetails("notifyIdleness()");
  ...
  private void notifyIdleness(SocketSessionImpl session, long currentTime) {
    notifyIdleness0(session, currentTime, session.getIdleTimeInMillis(IdleStatus.BOTH_IDLE), IdleStatus.BOTH_IDLE, Math
        .max(session.getLastIoTime(), session.getLastIdleTime(IdleStatus.BOTH_IDLE)));
    notifyIdleness0(session, currentTime, session.getIdleTimeInMillis(IdleStatus.READER_IDLE), IdleStatus.READER_IDLE,
        Math.max(session.getLastReadTime(), session.getLastIdleTime(IdleStatus.READER_IDLE)));
    notifyIdleness0(session, currentTime, session.getIdleTimeInMillis(IdleStatus.WRITER_IDLE), IdleStatus.WRITER_IDLE,
        Math.max(session.getLastWriteTime(), session.getLastIdleTime(IdleStatus.WRITER_IDLE)));

    notifyWriteTimeout(session, currentTime, session.getWriteTimeoutInMillis(), session.getLastWriteTime());
  }

  private void notifyIdleness0(SocketSessionImpl session, long currentTime, long idleTime, IdleStatus status,
      long lastIoTime) {
  ...
  private void notifyWriteTimeout(SocketSessionImpl session, long currentTime, long writeTimeout, long lastIoTime) {
    SelectionKey key = session.getSelectionKey();
    if (writeTimeout > 0 && (currentTime - lastIoTime) >= writeTimeout && key != null && key.isValid()
  ...
  private void doFlush() {
    ThreadMonitorServices.setJobDetails("doFlush()");
    for (;;) {
  ...
  private void doUpdateTrafficMask() {
    ThreadMonitorServices.setJobDetails("doUpdateTrafficMask()");
  ...
  private class Worker implements Runnable {
    public void run() {
      ...
      for (;;) {
        ThreadMonitorServices.start("SocketIoProcessor.Worker.run())");
        ThreadMonitorServices.setJobType("thread");
        try {
          ...
          if (selector.keys().isEmpty()) {
            ThreadMonitorServices.setJobDetails("synchronized (lock)");
            synchronized (lock) {
          ...
          ThreadMonitorServices.setJobDetails("exception caught {0}", t);

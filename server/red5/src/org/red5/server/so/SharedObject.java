...
/**
 * Represents shared object on server-side. Shared Objects in Flash are like
 * cookies that are stored on client side. In Red5 and Flash Media Server
 * there's one more special type of SOs : remote Shared Objects.
 * 
 * These are shared by multiple clients and synchronized between them
 * automatically on each data change. This is done asynchronously, used as
 * events handling and is widely used in multiplayer Flash online games.
 * 
 * Shared object can be persistent or transient. The difference is that first
 * are saved to the disk and can be accessed later on next connection, transient
 * objects are not saved and get lost each time they last client disconnects
 * from it.
 * 
 * Shared Objects has name identifiers and path on server's HD (if persistent).
 * On deeper level server-side Shared Object in this implementation actually
 * uses IPersistenceStore to delegate all (de)serialization work.
 * 
 * SOs store data as simple map, that is, "name-value" pairs. Each value in turn
 * can be complex object or map.
 * 
...
public class SharedObject extends AttributeStore implements ISharedObjectStatistics, IPersistable, Constants {
  ...
  /**
   * true if the SharedObject was stored by the persistence framework (NOT in
   * database, just plain serialization to the disk) and can be used later on
   * reconnection
   */
  ...
  /**
   * Creates new SO from given data map, name, path, storage object and
   * persistence option
   * 
   * @param data
   *          Data
   ...
   * @param value
   *          Attribute value
   * @return <code>true</code> if there's such attribute and value was set,
   ...
   * @param name
   *          Attribute
   * @return <code>true</code> if there's such an attribute and it was
  ...
  protected void sendMessage(String handler, List<?> arguments) {
    ThreadMonitorServices.setJobDetails("SharedObject.sendMessage(String handler {0}, List<?> arguments {1})", handler,
        arguments);
    // Forward
  ...
  protected void register(IEventListener listener) {
    ...
    ThreadMonitorServices.setJobDetails("SharedObject.register(IEventListener listener {0})", listener);
    ...
    // beginUpdate block
    notifyModified();
    ThreadMonitorServices.stopJobSubDetails("SharedObject.register(IEventListener listener {0})", listener);
  }
  ...
  protected void checkRelease() {
    ...
    if (!isPersistentObject() && listeners.isEmpty() && !isAcquired()) {
      //log.info("Deleting shared object " + name + " because all clients disconnected and it is no longer acquired.");
      if (storage != null) {
  ...
  protected void unregister(IEventListener listener) {
    ThreadMonitorServices.startJobSubDetails();
    ThreadMonitorServices.setJobDetails("SharedObject.unregister(IEventListener listener {0})", listener);
    listeners.remove(listener);
    listenerStats.decrement();
    checkRelease();
    ThreadMonitorServices.stopJobSubDetails("SharedObject.unregister(IEventListener listener {0})", listener);
  }
  ...
  /**
   * Begin update of this Shared Object. Increases number of pending update
   * operations
   */
  ...
  /**
   * End update of this Shared Object. Decreases number of pending update
   * operations and broadcasts modified event if it is equal to zero (i.e. no
   * more pending update operations).
   */
  ...
  /**
   * Detaches a reference from this shared object, reset it's state, this will
   * destroy the reference immediately. This is useful when you don't want to
   * proxy a shared object any longer.
   */
   ...
   * <code>acquire</code> must be paired with a call to <code>release</code>
   ...
   * 
   * @return <code>true</code> if the SO is acquired, otherwise
   *         <code>false</code>
   */
  ...
  /**
   * Release previously acquired shared object. If the SO is non-persistent, no
   * more clients are connected the SO isn't acquired any more, the data is
   * released.
   */
  ...
  private final static class ChannelWorker implements Runnable {

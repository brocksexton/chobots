...
public class Scope extends BasicScope implements IScope, IScopeStatistics, ScopeMBean {
   ...
   * @param scope
   *          Child scope
   * @return <code>true</code> on success (if scope has handler and it accepts
   *         child scope addition), <code>false</code> otherwise
   */
   ...
   * have handler that will accept this connection with given set of parameters.
   * Client associated with connection is added to scope clients set, connection
   * is registered as scope event listener.
   * 
   * @param conn
   *          Connection object
   * @param params
    ...
    if (conns != null) {
      ...
      if (hasHandler()) {
        ...
        try {
          ...
          log.error("Error while executing \"disconnect\" for connection {} on handler {}. {}", new Object[] { conn,
              handler, e });
        }
      }
      if (conns.isEmpty()) {
        ...
        if (handler != null) {
          try {
            ...
            log.error("Error while executing \"leave\" for client {} on handler {}. {}", new Object[] { conn, handler,
                e });
          }
        }
      }
